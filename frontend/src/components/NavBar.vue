...
document.addEventListener("login_finished", () => {
    initFlowbite()
})
    ...
    <div class="container mx-auto mt-[46px] px-0 pr-0">
        <div class="flex justify-between items-center xl:px-[45px] px-[25px]">
            ...
            <Transition name="slide-fade">
                <div v-if="menu"
                    class="fixed right-0 top-0 w-[230px] text-white bg-[#000] h-full z-30 flex gap-[15px] flex-col items-left text-[24px] p-[40px]">
                    <div class="flex justify-between items-center mb-[30px]">
                        <router-link tag="h1" class="font-bold text-[20px] flex gap-[15px] hover:text-black" to="/">
                    ...
                    <router-link tag="h1" active-class="activePage" class="hover:text-white"
                        to="/evenimente">Competiții</router-link>
                    <router-link tag="h1" active-class="activePage" class="hover:text-white"
                        to="/bilete">Bilete</router-link>
                    <router-link tag="h1" active-class="activePage" class="hover:text-white"
                        to="/castigatori">Câștigători</router-link>
                    ...
                    <div class="flex items-center relative justify-center gap-[30px] mt-[30px] w-full">
                        <router-link v-if="!account.isLogged()" :to="account.isLogged() ? '/account' : '/login'"
                            ...
                            class="text-white flex flex-row flex-nowrap items-center gap-[10px]"><font-awesome-icon
                                icon="fa-solid fa-user" class="fa-md" /></router-link>
                        <router-link v-if="!account.isLogged()" to="/register" tag="button"
                        ...
                        <div v-if="account.isLogged()" class="flex items-center gap-[20px]">
                            <router-link to="/cart" class="relative" tag="button">
                                <div
                                    class="absolute inline-flex items-center justify-center w-6 h-6 text-xs text-white bg-red-500 rounded-full -top-2 -right-2 dark:border-gray-900">
                                    {{ account.getStore().length }}</div>
            ...
            <div class="xl:flex flex-row gap-[50px] text-[20px] font-regular hidden w-full">
                <router-link tag="h1" active-class="activePage"
                    class="px-[15px] whitespace-nowrap hover:text-white hover:bg-black hover:border-b-[1px] hover:border-b-black"
                    to="/">Acasă</router-link>
                <router-link tag="h1" active-class="activePage"
                    class="px-[15px] whitespace-nowrap hover:text-white hover:bg-black hover:border-b-[1px] hover:border-b-black"
                    to="/evenimente">Competiții</router-link>
                <router-link tag="h1" active-class="activePage"
            ...
            <div class="xl:flex items-center relative justify-end gap-[30px] hidden w-full">
                <router-link v-if="!account.isLogged()" :to="account.isLogged() ? '/account' : '/login'" tag="button"
                    class="text-black flex flex-row flex-nowrap items-center gap-[10px]"><font-awesome-icon
                        icon="fa-solid fa-user" class="fa-xl" /></router-link>
                <router-link v-if="!account.isLogged()" to="/register" tag="button"
                    class="text-white bg-[#000000] text-[19px] px-[20px] py-[5px]">Înscrie-te</router-link>
                <router-link v-if="!account.isLogged()" to="/register" tag="button"
                ...
                <div v-if="account.isLogged()" class="flex items-center gap-[20px]">
                    <router-link to="/cart" class="relative" tag="button">
                        <div
                            class="absolute inline-flex items-center justify-center w-4 h-4 text-xs text-white bg-red-500 rounded-full -top-2 -right-2 dark:border-gray-900">
                            {{ account.getStore().length }}</div>
                ...
                <div v-if="account.isLogged()" id="dropdownAvatarName"
                    class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-52 dark:bg-gray-700 dark:divide-gray-600">
                    <div class="px-3 py-3 text-sm text-gray-900 dark:text-white">
                        <div class="font-medium text-[16px]" :title="account.getUsername()">{{ account.getUsername() }}
                        </div>
                        <div class="truncate" :title="account.getEmail()">{{ account.getEmail() }}</div>
                    </div>
                    <ul class="py-2 text-sm text-gray-700 dark:text-gray-200"
                    ...
                    <div class="py-2">
                        <a @click="$event => account.logOut()"
                            class="text-[14px] text-gray-700 cursor-pointer flex items-center gap-[8px] px-3 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white hover:text-black">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"
                                style="fill: rgba(55, 65, 81, 1);transform: ;msFilter:;">
                                <path d="M16 13v-2H7V8l-5 4 5 4v-3z"></path>
                                <path
