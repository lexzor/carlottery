        ...
        <div class="flex justify-between items-center px-[45px]">
            ...
            <div class="flex items-center relative gap-[30px]">
                ...
                <!-- <div v-if="account.isLogged()"> -->
                    <button :class="displayAccBtn" id="dropdownDefaultButton" data-dropdown-toggle="dropdown" class="text-white bg-[#000000] px-[20px] py-[5px] whitespace-nowrap flex items-center justify-center gap-[10px]" type="button">
                        <font-awesome-icon icon="fa-solid fa-user" class="fa-xl" />
                        <h1  class="text-[19px] h-fit text-center">{{ account.getUsername() }}</h1>
                    </button>
                    <!-- Dropdown menu -->
                    <div id="dropdown" :class="displayAccBtn" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
                        <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                            <li v-if="account.isOwner()">

                            </li>
                            <li>
                                <a href="#" @click="account.logOut()" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Log out</a>
                            </li>
                        </ul>
                    </div>
            </div>
