...
const getUser = async (id) => {
    await axios.post("http://localhost/loterie/getUser.php", {
        id: id
    }, {
        headers: {
...
const editUser = async () => {
    ...
    await axios.post("http://localhost/loterie/editUser.php", userObj, {
        headers: {
            "Content-Type": "application/json"
        }
    }).catch(err => {
...
<template>
    <div class="p-[100px]">
        <h1 class="text-center mb-[80px] text-[20px]">De aici poti modifica contul utilizatorului selectat <b>[ID: {{ user.id }}]</b></h1>
        <div class="max-w-[400px] mx-auto flex flex-col gap-[30px]">
            ...
            <MazBtn @click="editUser" :loading="sending ? true : false" class="mx-auto">Actualizeaza</MazBtn>
        </div>
    </div>
</template>